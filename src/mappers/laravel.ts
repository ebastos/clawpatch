...
function controllerGroupRoutes(
  ...
  for (const statement of routeStatements(body)) {
    const route = routeFromCalls(file, imports, parseRouteCalls(statement), groupPrefixes, controllerClass);
    if (route !== null) {
      routes.push(route);
    }
  }
...
function routeFromCalls(
  ...
  const target =
    controllerClassOverride === null
      ...
      : { controllerClass: controllerClassOverride, action: stringLiteralValue(call.args[1] ?? "") };
  if (target === null) {
    return null;
  }
  return {
    file,
    method: call.name,
    uri: routeUriWithPrefixes([...basePrefixes, ...routePrefixesFromCalls(calls.slice(0, routeIndex))], uri),
...
function routeTarget(
  ...
  const controllerClass = resolveImportedClassName(imports, classLiteralName(targetArgs[0] ?? "") ?? "");
